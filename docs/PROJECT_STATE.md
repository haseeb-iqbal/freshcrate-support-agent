...
`search_knowledge_base`, `lookup_order` (one order — resolved via `selectOrder`, a pure resolver in `lib/tools/select-order.ts` that takes `order_number` OR `position` (1 = most recent, 2 = 2nd most recent, …) OR `kind` (subscription|extra) OR `status` OR `date` (YYYY-MM-DD, matched against `deliveryDate` or the placed-day, resolved by the model from the # Today date in the prompt), so "my 2nd last order", "my most recent extra meal", or "my order on 25th July" all resolve without dumping full history to the model), `list_orders` (history → card; details withheld from the model), `get_subscription` (live status — must be called for status/plan/billing/pause questions), `pause_subscription`, `resume_subscription`, `reactivate_subscription`, `cancel_subscription`, `change_plan`, `change_dietary_track`, `issue_refund`, `escalate_to_human`.
...
- **Pause (propose→confirm):** 1–52 weeks, a target date within a year, or **indefinite**. Takes effect next week. Deferred to the next monthly bill at the full weekly rate (round-trips to zero on an immediate pause+resume); the $8/week fee still applies while paused across a billing date. The credit is `weeksToBilling` × weekly rate, stored as a one-time `billingAdjustmentCents` adjustment rather than a transaction row written up front. Thereafter **every** pause — finite or indefinite — is billed $8/week at each billing date for as long as it runs ($32 for a full month, pro-rated for a finite pause's last partial month). A cancelled sub can't be paused; a pause can be resumed early.
- **Resume (paused, propose→confirm):** defers a charge of `weeksToBilling × weekly` - at the NEW plan's weekly rate if switching plan at the same time (`new_plan`) - to the next monthly bill via the same `billingAdjustmentCents` field. Takes effect next week.
...
```bash
npm run test:e2e             # db:reset + dev:mock + headless Cypress (36 specs across 4 files) in one command
# npm run cypress:open       # interactive runner — run `npm run db:reset` first
```
`MockChatProvider` is gated behind `MOCK_LLM=1` (never active in production) and scripts canned tool-call/text turns keyed off the incoming message, so all 36 E2E specs run without hitting OpenAI: 19 read-only in `agent.cy.ts` (menu question with a source, positional order lookup, order-history double-text fix, pause, resume card, plan-change-while-paused redirect, over-ceiling refund escalation, 14-day refund-cooldown escalation, confirmable refund card, off-topic refusal, ambiguous-cancel clarification, dietary-track switch confirmation card, rendered markdown, stripped inline citation, the two confirmation-outcome-payload cases, the single-order card for a positional lookup, subscription-cost routing, and the single-order card for a date-based lookup) plus 5 in `chips.cy.ts` (shared prompt chips and their answers) plus 8 in `ui.cy.ts` (preview-note hover/focus/click/second-click/Escape, the My Account label, route-driven tab titles, and the slug-free help center) that are all read-only, plus 4 in `confirm.cy.ts` that click Confirm/Not now and therefore **mutate the DB** (pause confirm, pause decline, refund confirm, dietary-track switch confirm) — hence the reseed baked into `test:e2e`.
