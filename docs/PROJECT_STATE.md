...
- **Pause (propose→confirm):** 1–52 weeks, a target date within a year, or **indefinite**. Takes effect next week. Credits `min(pauseWeeks, weeksToBilling)` (indefinite → `weeksToBilling`) × (weekly − **$8/week** pause fee); indefinite bills the $8/week monthly while paused. Writes a `pause_credit` ledger row. A cancelled sub can't be paused.
